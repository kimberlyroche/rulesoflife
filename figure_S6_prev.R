...
use_Frobenius <- TRUE # otherwise, use Riemannian
...
for(i in 1:N) {
  if(use_Frobenius) {
    ...
    D2[i] <- dist4cov(F2$Sigmas[,,i] + diag(D)*1e-06,
                      F2$mean + diag(D)*1e-06)$dist
    D3[i] <- dist4cov(F3$Sigmas[,,i] + diag(D)*1e-06,
                      F3$mean + diag(D)*1e-06)$dist
  } else {
...
ggsave(file.path("output", "figures", paste0("S6_", ifelse(use_Frobenius, "Fro", ""), ".png")),
       dpi = 100,
       units = "in",
       height = 4,
       width = 6)
