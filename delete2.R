...
p4 <- ggraph(graph, layout = "fr") +
  ...
  geom_node_label(aes(label = taxon_idx), size = 2, repel = TRUE, label.padding = unit(0.15, "lines")) +
...
p2_padded <- plot_grid(NULL, p2, ncol = 2,
                       rel_widths = c(-0.05, 1))
p2_padded <- plot_grid(NULL, p2_padded, NULL, ncol = 1,
                       rel_heights = c(0.05, 1, -0.02))

p3_padded <- plot_grid(NULL, p3, ncol = 2,
                       rel_widths = c(0.1, 1))
p5_padded <- plot_grid(NULL, p5, ncol = 2,
                       rel_widths = c(0.05, 1))

prow2 <- plot_grid(p2_padded, p3_padded, p5_padded, ncol = 3,
                   rel_widths = c(0.7, 0.7, 1),
                   labels = c("C", "D", "E"),
                   label_size = 19,
                   label_x = -0.05,
                   label_y = 1.04)
