...
for(pair_name in names(named_pairs)) {
  ...
  mean_pred <- Theta_u + (Lambda_o-Theta_o)%*%solve(Gamma_oo, Gamma_ou)

  # convert to CLR
  dim(mean_pred)
  test <- clr_array(alrInv_array(mean_pred, fit$alr_base, 1), 1)


  sname <- host
  pred_before <- aligned_obj %>%
    filter(host == sname & coord == 2)
  plot(pred_before$day, pred_before$mean, type = "l")
  lines(pred_before$day, test[2,], col = "red")
